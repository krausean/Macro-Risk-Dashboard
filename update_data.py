...
HEADERS={"User-Agent":"PersonalMacroRiskDashboard/3.0 (non-commercial educational use)"}
...
def num(v):
    try:
        ...
        if s.lower() in {"","n/a","na","nd",".","null","none","bank holiday"}:return None
        return float(s)
    except:return None

def chg(a,n=20):return None if len(a)<=n else a[-1]["value"]-a[-1-n]["value"]
def lvl(v,stops):
...
def fetch_treasury(kind,years,columns):
    rows=[]
    for year in years:
        reader=csv.DictReader(io.StringIO(get(treasury_csv(kind,year)).text))
        for r in reader:
...
def latest_sloos_release():
    page="https://www.federalreserve.gov/data/sloos.htm";soup=BeautifulSoup(get(page).text,"html.parser");links=[]
...
def fetch_sloos():
    release=latest_sloos_release();soup=BeautifulSoup(get(release.replace(".htm","-chart-data.htm")).text,"html.parser")
    for table in soup.find_all("table"):
        txt=table.get_text(" ",strip=True).lower()
        if "net percentage" not in txt or "tightening standards" not in txt or "c&i" not in txt:continue
        out=[]
        for tr in table.find_all("tr"):
            cells=[c.get_text(" ",strip=True) for c in tr.find_all(["th","td"])]
            if len(cells)<3 or not re.fullmatch(r"\d{4}:\d",cells[0].replace(" ","")):continue
            v=num(cells[1])
...
def fetch_boc_csv(series,lookup_page,days=800):
    end=datetime.now(timezone.utc).date()
    start=end-timedelta(days=days)
    url="https://www.bankofcanada.ca/stats/results/csv"
    params={
      "ByDate_frequency":"daily","dF":start.isoformat(),"dT":end.isoformat(),
      "lP":lookup_page,"rangeType":"dates","sR":"2010-04-21","se":f"LOOKUPS_{series}"
    }
    text=get(url,params=params).text
    rows=list(csv.reader(io.StringIO(text)))
    header_i=None;date_i=None;series_i=None
    for i,row in enumerate(rows):
        clean=[x.strip().lstrip("\ufeff") for x in row]
        lowered=[x.lower() for x in clean]
        if "date" in lowered and any(x.upper()==series.upper() for x in clean):
            header_i=i;date_i=lowered.index("date");series_i=next(j for j,x in enumerate(clean) if x.upper()==series.upper());break
    if header_i is None:raise RuntimeError(f"Bank of Canada CSV header not found for {series}")
    out=[]
    for row in rows[header_i+1:]:
        if len(row)<=max(date_i,series_i):continue
        ds=row[date_i].strip();v=num(row[series_i])
        if not re.fullmatch(r"\d{4}-\d{2}-\d{2}",ds) or v is None:continue
        out.append({"date":ds,"value":v})
    if not out:raise RuntimeError(f"No Bank of Canada CSV observations for {series}")
    return out
...
def metric(name,group,v,unit,level,why,points=None,threshold=None,change=None,change_unit=None,change_label="",decimals=2,secondary=None):
    return {"name":name,"group":group,"latest":v,"unit":unit,"level":level,"why":why,"points":points,"threshold":threshold,
            "change":change,"change_unit":change_unit if change_unit is not None else unit,"change_label":change_label,"decimals":decimals,"secondary":secondary}
def band(score):
...
def main():
    ...
    try:
        yrs=[now.year-1,now.year];nom=fetch_treasury("daily_treasury_yield_curve",yrs,["2 Yr","10 Yr"]);real=fetch_treasury("daily_treasury_real_yield_curve",yrs,["10 YR"])
        u2=[{"date":r["date"],"value":r["2 Yr"]} for r in nom if r["2 Yr"] is not None];u10=[{"date":r["date"],"value":r["10 Yr"]} for r in nom if r["10 Yr"] is not None];r10=[{"date":r["date"],"value":r["10 YR"]} for r in real if r["10 YR"] is not None]
        ...
        curve=curves[-1]["value"];recent=[x["value"] for x in curves[-126:]];resteep=bool(recent and min(recent)<0 and curve>=0);realv=r10[-1]["value"];move=chg(u10,20)
        p_real=1.5 if realv>=2.5 else 1 if realv>=2 else .5 if realv>=1.5 else 0
        p_move=1 if move is not None and move>=.75 else .5 if move is not None and move>=.4 else 0
        p_curve=.5 if resteep or curve<0 else 0
        score+=p_real+p_move+p_curve
        metrics += [
          metric("U.S. 10-Year Treasury","RATES",u10[-1]["value"],"%",lvl(abs(move or 0),[(.4,"green"),(.75,"yellow"),(99,"orange")]),"A rapid rise tightens financial conditions; the absolute yield alone is not a recession call.",p_move,"Watch if the 10Y rises ≥0.40 pp over roughly 20 trading sessions.",move," pp","/~20 sessions"),
          metric("U.S. 10-Year Real Yield","RATES",realv,"%",lvl(realv,[(1.5,"green"),(2,"yellow"),(2.5,"orange"),(99,"red")]),"Higher real yields raise the inflation-adjusted hurdle rate for borrowing and risky assets.",p_real,"1.5% watch; 2.0% elevated; 2.5%+ high.",chg(r10,20)," pp","/~20 sessions"),
          metric("U.S. 10Y–2Y Curve","CURVE",curve," pp","yellow" if curve<0 or resteep else "green","An inversion is a cycle-warning signal; re-steepening after inversion can also merit attention.",p_curve,"Negative (inverted) is a warning; recent re-steepening after inversion also counts.",secondary="Re-steepened after inversion" if resteep else "Inverted" if curve<0 else "Positive slope")]
    ...
    try:
        ...
        p_s=3 if s>=.5 else 2 if s>=.3 else 1 if s>=.2 else 0
        p_pg=2 if pg<0 else 1 if pg<50 else .5 if pg<100 else 0
        p_inf=1 if inf>=3.5 else .5 if inf>=2.5 else 0
        score+=p_s+p_pg+p_inf
        metrics += [
          metric("U.S. Unemployment Rate","LABOUR",u[-1]["value"],"%",lvl(s,[(.2,"green"),(.3,"yellow"),(.5,"orange"),(99,"red")]),"The dashboard focuses on how quickly unemployment is rising from its cycle low.",p_s,"Sahm-style deterioration: 0.20 pp watch; 0.30 pp elevated; 0.50 pp high.",secondary=f"Sahm-style deterioration: {s:.2f} pp"),
          metric("3-Month Avg Payroll Growth","LABOUR",pg,"k",lvl(pg,[(0,"red"),(50,"orange"),(100,"yellow"),(1e9,"green")]),"A three-month average smooths noisy monthly job reports and shows whether hiring momentum is fading.",p_pg,"100k+ calm; 50–100k watch; 0–50k elevated; below zero high.",decimals=0,secondary="Average monthly change"),
          metric("U.S. CPI Inflation","INFLATION",inf,"%",lvl(inf,[(2.5,"green"),(3.5,"yellow"),(5,"orange"),(99,"red")]),"Inflation matters here mainly because it can limit how quickly the Fed can cut rates during a slowdown.",p_inf,"Below 2.5% calm; 2.5–3.5% watch; 3.5–5% elevated; 5%+ high.",decimals=1,secondary="Year over year")]
    ...
    try:
        a,release=fetch_sloos();tight=a[-1]["value"];p_sl=3 if tight>=40 else 2 if tight>=20 else 1 if tight>=10 else .5 if tight>0 else 0;score+=p_sl
        metrics.append(metric("C&I Lending Standards — Net Tightening","CREDIT",tight,"%",lvl(tight,[(.1,"green"),(10,"yellow"),(20,"orange"),(999,"red")]),"Negative means banks are easing on net; a large positive number means credit is becoming harder for businesses to obtain.",p_sl,"10%+ deserves attention; 20%+ is meaningful tightening; 40%+ is severe.",secondary=f"{a[-1]['period']} · negative = easing"))
    ...
    try:
        ca10=fetch_boc_csv("V39055","lookup_bond_yields.php");boc=fetch_boc_csv("V39079","lookup_canadian_interest.php")
        metrics += [
          metric("Canada 10-Year Benchmark","CANADA",ca10[-1]["value"],"%",lvl(abs(chg(ca10,20) or 0),[(.35,"green"),(.6,"yellow"),(99,"orange")]),"Canadian long yields provide local context for mortgage, financing and CAD investment conditions.",None,"Context only. A move of ~0.35 pp over 20 observations is worth watching.",chg(ca10,20)," pp","/~20 observations"),
          metric("Bank of Canada Policy Rate","CANADA",boc[-1]["value"],"%","green","Canada's key policy rate influences short-term borrowing costs and helps frame Canadian financial conditions.",None,"Context only; no points are added to the U.S.-led composite score.",secondary="Target overnight rate")
        ];status.append({"name":"Bank of Canada","ok":True,"note":"official Bank CSV export"})
    except Exception as e:status.append({"name":"Bank of Canada","ok":False,"note":str(e)[:90]})

    score=min(10,score);b=band(score);P=float(CONFIG["portfolio_value"]);bits=[]
    if facts.get("sahm") is not None:bits.append(f"labour deterioration {facts['sahm']:.2f} pp")
    if facts.get("sloos") is not None:bits.append(f"C&I net tightening {facts['sloos']:.1f}%")
    if facts.get("real10") is not None:bits.append(f"10Y real yield {facts['real10']:.2f}%")
    explanation="Composite score uses only source groups that refreshed successfully today. "+(("Key readings: "+", ".join(bits)+". ") if bits else "")+"No single indicator is treated as a recession call."
    ...
    checks=[
      ...
      {"title":"Banks tightening C&I standards > 20%","detail":f"Latest SLOOS: {facts['sloos']:.1f}% (negative means easing)." if facts.get("sloos") is not None else "SLOOS data unavailable.","active":facts.get("sloos") is not None and facts["sloos"]>20},
